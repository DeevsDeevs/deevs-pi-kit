...
describe("collaborator Auto mode", () => {
	...
	it("serializes Auto starts and fails closed on a stale lock", async () => {
		...
		writeFileSync(lockPath, `${JSON.stringify({ token: "lock_00000000-0000-4000-8000-000000000001", pid: 999999 })}\n`);
		...
		writeFileSync(lockPath, `${JSON.stringify({ token: "lock_00000000-0000-4000-8000-000000000002", pid: process.pid })}\n`);
		expect(store.recoverStartLock()).toBe(true);
		expect(store.recoverStartLock()).toBe(false);
		writeFileSync(lockPath, `${JSON.stringify({ token: "lock_00000000-0000-4000-8000-000000000003", pid: process.pid, unexpected: true })}\n`);
		expect(() => store.recoverStartLock()).toThrow("malformed");
		writeFileSync(lockPath, `${JSON.stringify({ token: "", pid: process.pid })}\n`);
		expect(() => store.recoverStartLock()).toThrow("malformed");
		writeFileSync(lockPath, `${JSON.stringify({ token: "lock_00000000-0000-4000-8000-000000000004", pid: process.pid, identity: null })}\n`);
		expect(() => store.recoverStartLock()).toThrow("malformed");
