...
Universal messaging is staged, not released: the [MCP compatibility probe](extensions/runtime/MCP-STAGE0.md) passed for its recorded restricted provider configurations. The [current MCP implementation](extensions/runtime/MCP-PLAN.md) provides six messaging tools, restricted Runtime credentials, namespace-bound retrieval offers, atomic replies and durable retry receipts through one [shared skill](skills/collaborator-messaging/SKILL.md). Runtime accepts only schema v11; unsupported stores are rejected without migration or deletion. Native provider-startup configuration, reference wakes, durable Pi receive admission and the live three-harness ring remain unfinished. The old direct Pi messaging registrar and ordinary-mail full-body native delivery are removed. Ordinary mail cannot enter native claims, ACKs or replay; Monitor and supported task delivery remain separate. Pi reference notification is the next implementation slice; native automatic wakes remain blocked.
