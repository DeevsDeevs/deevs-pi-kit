...
npm run smoke:native-live                # authenticated real Claude/Codex read-only, write, and resume gate
```

`npm run check` runs typechecking, tests, RPC/print/JSON mode smokes, the lockfile audit, and a package dry run. The isolated Runtime/Collaborator gates are separate because they start real Herdr and Pi processes; the deterministic native gate uses executable shims through the production Runtime/runner boundary. `smoke:native-live` is a release-only authenticated gate that records installed CLI versions and runs real Claude Code and Codex read-only, workspace-write, and resume behavior in a disposable Git workspace.
