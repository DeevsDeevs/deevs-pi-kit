...
Runtime accepts only schema v13, without migration or legacy messaging fallback. Pi's idle, empty-TUI reference hints are best-effort and never automatically replay after uncertainty; RPC/headless modes do not provide editor authority. Automatic native wakes remain blocked. Client receipts are not native ACKs, provider commits or fsync-based admission. General descriptor repair and daemon delivery handoff remain unproven. See the [current messaging contract and limits](extensions/runtime/PROTOCOL.md#shared-mcp-messaging). Historical plans and live-run evidence remain available in Git history.
