...
Universal messaging is staged, not released: the [MCP compatibility probe](extensions/runtime/MCP-STAGE0.md) passed for its recorded restricted provider configurations. The [current MCP implementation](extensions/runtime/MCP-PLAN.md) provides six messaging tools, restricted Runtime credentials, namespace-bound retrieval offers, atomic replies and durable retry receipts through one [shared skill](skills/collaborator-messaging/SKILL.md). Runtime accepts only schema v10; unsupported stores are rejected without migration or deletion. Native provider-startup configuration, reference wakes, durable Pi receive admission and the live three-harness ring remain unfinished. The old direct Pi messaging registrar is removed; remaining full-body delivery is replacement work, not a supported compatibility fallback.
