...
Checkpoint state is restored after resume/tree navigation. At 80% context usage, Pi must save one concise Chain link before any other work: non-`chain_save` tools are blocked and an ignored reminder receives one hidden follow-up turn. After the checkpoint is saved, Pi's native compactor retains full lifecycle control; Chain never starts or aborts a turn for compaction. Native compaction resets the one-shot threshold and reminds the first post-compaction turn to reload the saved Chain. Successful Chain tools update state directly. Descendant advances of repository HEAD are detected without parsing shell commands, while sideways checkouts and resets are ignored. Mission lifecycle changes, explicit Mission milestones/review adjudication, Chain forks, and write-enabled Subagents also mark checkpoints due. Ordinary edits and bounded Jobs do not: activity is not automatically a durable milestone.
