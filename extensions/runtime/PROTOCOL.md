...
Runtime state v3 discriminates strict Pi targets from generic bridge targets. Existing v1/v2 targets migrate to `kind: "pi"` without changing their keys, sessions, participants, mailbox events, claims, or wakes; `pi.register` retains its canonical session-file and authoritative Herdr Pi checks.
