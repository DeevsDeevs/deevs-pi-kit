...
A bridge must first report the generic Herdr session identity `(source=pi-kit-bridge, agent=bridge, kind=id, value=launchId)`. Registration re-verifies the exact pane, terminal, single tab/workspace binding, cwd, caller generation, participant prior generation, and capability digest before one atomic state mutation creates the bridge target, acquires its preallocated holder generation, and consumes the launch token. The launch token cannot register again. The separate reconnect credential deterministically rebuilds the same in-memory registration after Runtime restart only while the exact bridge target still holds that participant generation. Opaque metadata is capped at 16 entries / 1 KiB each and rejects driver/model/persona/profile or credential-like keys; those values remain in their authoritative private/runtime-specific stores instead of leaking into generic target state. Heartbeat and stop dispatch by target kind; bridge stop re-verifies the exact reported bridge and single-pane tab before closing it. Plaintext launch/reconnect secrets are absent from durable state, bridge metadata, and registration results.

This authority protects against accidental/cross-wired same-UID project children that do not possess the random credential; it is not an OS sandbox against a hostile same-UID process that can inspect another process's memory. Native Claude Code/Codex launchers and runners are not shipped by this seam, so their driver selectors still fail before confirmation.

### Durable common bridge runner

The internal bridge runner is a persistent Herdr-hosted controller with a private, frozen driver registry; the shipped validation adapter is `fake`, not a user-selectable collaborator driver. Claude Code and Codex remain unavailable until their adapters pass later profile gates.
...
Each turn uses a fresh detached Node worker process group. The worker persists and reports its own PID/start identity, then waits for the controller to durably record and explicitly authorize that exact worker before spawning one non-detached native child into the recorded group. Readiness failure quiesces the exact group or enters `needs_attention`. Worker/native environments are explicit allowlists and contain no Runtime socket credentials, launch/reconnect token, registration key, Herdr authorization, or controller environment. The fake adapter only produces argv/stdin/env and validates typed frames; common code owns fatal UTF-8/JSONL framing, closed frame schemas, line/total/stderr/frame/body/wall caps, terminal uniqueness, process identity, cancellation, and persistence.

A zero exit without exactly one validated terminal frame is failure. Terminal/session cursor is durable before outbox send. Reply send ID/body are durable before `mailbox.send`; uncertain publication retries the exact ID/body and relies on Runtime's sender/send-ID fingerprint dedupe.

Native computation is honestly **at least once**. A crash after execution may have begun but before terminal persistence records `sessionAdvance: uncertain`. The runner resumes an exact live worker or its durable terminal result; a missing or mismatched worker identity fails closed rather than risking duplicate effects. Cancel intent is durable before signaling. If the worker leader identity is missing/mismatched or its group cannot be proven quiescent, the turn and controller enter typed `needs_attention`; Runtime never guesses a PID/PGID, replays, replies, ACKs a later claim, or claims exactly-once effects.
