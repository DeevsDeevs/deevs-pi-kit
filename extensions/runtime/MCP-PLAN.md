...
The direct Pi batch/status messaging registrar and its handlers are removed, with no argument translation. Full-body delivery still present is **unfinished replacement work**, not an approved release fallback. Native provider provisioning, reference wakes and Pi persisted-body admission remain incomplete.
...
1. **Native provider provisioning:** default Pi messaging registration is now shared MCP; complete native MCP configuration through verified launch/registration using restricted settings, without global configuration changes. Remove remaining full-body delivery. No old argument translation or `legacy_push` mode.
2. **Reference wakes:** the user approved extending Herdr with atomic expected-identity/idle/unfocused/input-empty admission guards. Installed Herdr 0.9.0/protocol22 exposes only target/text/wait and permits already-working submission; Runtime prechecks alone cannot close that race. Implement and prove the guarded host API before automatic native wakes; installation remains separate. Partition ordinary MCP mail from native Monitor admission and separately supported tasks. No second `inbox.claim` consumer. Persist exact event/target/holder/client/controller attempts before Herdr submission. Ambiguous/submitted attempts are not automatically replayed; idle/focus/human-priority gates remain. The launching Pi is the first submitter; daemon handoff follows with service-enforced owner fencing.
