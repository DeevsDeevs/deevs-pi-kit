...
- `service/state.ts`: current state version **11 only**, atomic publication/operation receipts, immutable recipient binding, retrieval offers and client receipts. No v1–v10 migration. Ordinary mail cannot carry native claim/admission evidence.
...
The direct Pi batch/status messaging registrar and its handlers are removed, with no argument translation. Ordinary-mail full-body native delivery and automatic native prompt injection are removed. Monitor and supported task claims remain separate. Native provider provisioning, reference wakes and Pi persisted-body admission remain incomplete.
...
- Ordinary-mail retention does not use native ACK. Publication receipts and retrieval offers protect bodies; age-based cleanup requires an expired recipient namespace (or an unbound event) and no retained protection. Namespace expiry clears offers/receipts and retains a terminal tombstone. Retention may expire namespaces already older than its cutoff; clock rollback cannot resurrect them.
