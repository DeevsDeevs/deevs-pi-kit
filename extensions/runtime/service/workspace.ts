...
function bounded(value: string, name: string, max: number): string { if (!value.trim() || Buffer.byteLength(value) > max) throw new HostedWorkspaceError("invalid_request", `${name} is invalid.`); return value; }
...
function workspaceError(cause: unknown): HostedWorkspaceError { return cause instanceof HostedWorkspaceError ? cause : cause instanceof RuntimeGitError ? new HostedWorkspaceError("git_error", cause.message) : new HostedWorkspaceError("git_error", cause instanceof Error ? cause.message : String(cause)); }
