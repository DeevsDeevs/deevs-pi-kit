...
export class RuntimeGit {
	...
	async createWorktree(repository: RuntimeRepository, path: string, branchRef: string, baseCommit: string, kind: "collab" | "integrate" = "collab"): Promise<RuntimeWorktreeIdentity> {
		...
		return this.verifyWorktree(repository, path, branchRef, base);
	}
	...
	async finalize(repository: RuntimeRepository, expectedMainHead: string, preparedHead: string): Promise<string> {
		...
		await this.run(repository.root, ["merge", "--ff-only", "--no-edit", "--no-verify", prepared]);
	...
	async removeWorktree(repository: RuntimeRepository, worktree: RuntimeWorktreeIdentity): Promise<void> {
		...
		try { verified = await this.verifyWorktree(repository, worktree.path, worktree.branchRef, worktree.headCommit); }
			...
			const registered = (await this.worktrees(repository.root)).some((entry) => entry.path === worktree.path);
			const refExists = (await this.runCodes(repository.root, ["show-ref", "--verify", "--quiet", worktree.branchRef], [0, 1])).code === 0;
			if (!registered && !refExists) return;
			throw error;
		}
		...
		if ((await this.worktrees(repository.root)).some((entry) => entry.path === verified.path)) throw new RuntimeGitError("Git still registers the removed worktree.");
		await this.run(repository.root, ["update-ref", "-d", verified.branchRef, verified.headCommit]);
	}
	...
	async discardWorktree(repository: RuntimeRepository, worktree: RuntimeWorktreeIdentity): Promise<void> {
		...
		if ((await this.worktrees(repository.root)).some((entry) => entry.path === verified.path)) throw new RuntimeGitError("Git still registers the discarded worktree.");
		await this.run(repository.root, ["update-ref", "-d", verified.branchRef, verified.headCommit]);
	}
	...
	private async worktrees(cwd: string): Promise<RuntimeWorktreeIdentity[]> {
		...
		const result: RuntimeWorktreeIdentity[] = [];
		let current: Partial<RuntimeWorktreeIdentity> = {};
		for (const value of values) {
			if (!value) {
				if (current.path && current.branchRef && current.headCommit) result.push(current as RuntimeWorktreeIdentity);
				current = {};
				continue;
			}
	...
	private runCodes(cwd: string, args: string[], allowed: number[], stdin?: string | Buffer, extraEnv?: NodeJS.ProcessEnv): Promise<{ code: number; stdout: Buffer; stderr: Buffer }> {
		return new Promise((resolvePromise, reject) => {
			const child = execFile("git", gitArgs(args), { cwd, env: gitEnvironment(extraEnv), encoding: "buffer", maxBuffer: MAX_GIT_BUFFER, timeout: 30_000 }, (error, stdout, stderr) => {
