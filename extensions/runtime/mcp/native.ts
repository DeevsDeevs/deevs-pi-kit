...
export function nativeMessagingLaunch(input: NativeMessagingInput): NativeMessagingLaunch {
	...
	const serverConfig = `mcp_servers.${serverName}={command=${JSON.stringify(server.command)},args=${JSON.stringify(server.args)}}`;
	const startupMessage = "Acknowledge this collaborator workflow and wait for operator input."
		+ ` No tools or other work are requested by this startup message. ${context}`;
	const args = input.driver === "claude-code"
		...
		: ["--sandbox", "workspace-write", "--config", serverConfig, ...modelArgs, "--", startupMessage];
	...
	const configuration = {
		...
		policy: "native-user-configuration",
		args,
