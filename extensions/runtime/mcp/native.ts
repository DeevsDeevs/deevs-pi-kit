...
export function nativeMessagingLaunch(input: NativeMessagingInput) {
	...
	const context = [input.personaPrompt, skill, "Runtime may finish provisioning this connection after startup. Wait for explicit operator input before using its messaging tools. An unavailable descriptor is pending setup, not permission to invent another namespace or client."].filter(Boolean).join("\n\n");
	...
	const args = input.driver === "claude-code"
		...
		: ["--sandbox", "workspace-write", "--config", `mcp_servers.${serverName}={command=${JSON.stringify(server.command)},args=${JSON.stringify(server.args)}}`, ...modelArgs, "--", `Acknowledge this collaborator workflow and wait for operator input. No tools or other work are requested by this startup message.\n\n${context}`];
