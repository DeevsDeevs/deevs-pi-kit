...
- Submit `changes_requested` only when at least one blocker or major finding exists. Every blocker/major must name either the exact zero-based Mission `requirementIndex` it violates or a typed `criticalImpact` of `security` or `data_loss`. When the task supplies accepted findings and enforced correction paths, a requirement index must be among those accepted and the finding must name one enforced path; new security/data-loss findings still count. Otherwise classify it as minor/nit follow-up. Minor and nit findings are non-blocking backlog, so submit `clear` for those alone.
