...
- Submit `changes_requested` only when at least one blocker or major finding exists. Every blocker/major must name either the exact zero-based Mission `requirementIndex` it violates or a typed `criticalImpact` of `security` or `data_loss`; when the task supplies enforced correction paths, it must also name one of those exact paths. Otherwise classify it as minor/nit follow-up. Minor and nit findings are non-blocking backlog, so submit `clear` for those alone.
