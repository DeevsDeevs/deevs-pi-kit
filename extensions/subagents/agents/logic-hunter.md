...
Use `safe_read` for exact source inspection, `safe_search` for precise code lookup, and `safe_list` for targeted navigation. Prefer narrow searches over broad repository sweeps; ask the parent to run a focused check when execution evidence is required.
