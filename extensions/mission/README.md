...
Mission creation starts review-due and becomes `not_required` only after its initial workspace fingerprint is durably admitted. Later material mutation sets independent review due and starts a short durable quiet window before admission. Mission then reserves one candidate before launching a fresh read-only `reviewer`; the child submits findings through the schema-validated `review_report` tool, and the parent adjudicates that exact run. A candidate that changes while review runs is typed `superseded` and does not consume reviewer-failure capacity; only typed launch, runtime, missing-run, malformed-report, or fingerprint failures do. The first review covers the typed Mission scope. After an adjudicated candidate, Runtime persists the structured accepted findings and requires one clean correction commit. It records that commit's exact base/head revisions and changed paths, preserves prior evidence for untouched areas, and supplies only that bounded diff to the correction reviewer. Runtime counts `blocker|major` only when the report names an enforced changed path and links the finding to an accepted requirement index or a typed `security|data_loss` impact; out-of-scope discoveries are structurally downgraded to non-blocking backlog. The fourth valid correction cycle blocks, and trusted UI authorization permits exactly one additional cycle rather than reopening an unbounded review window. Missing or malformed reports remain `unknown`, never implicit clearance. Completion is vetoed until:
