...
Mission creation starts review-due and becomes `not_required` only after its initial workspace fingerprint is durably admitted. Later material mutation sets independent review due and starts a short durable quiet window before admission. Mission then reserves one candidate before launching a fresh read-only `reviewer`; the child submits findings through the schema-validated `review_report` tool, and the parent adjudicates that exact run. A candidate that changes while review runs is typed `superseded` and does not consume reviewer-failure capacity; only typed launch, runtime, missing-run, malformed-report, or fingerprint failures do. Every review requires a clean committed candidate, and the first review covers the typed Mission scope while persisting each repository's exact reviewed head. After changes are requested, Runtime persists the structured accepted findings, verifies those reviewed heads remain ancestors, and derives the complete changed path set from each accepted head to the new clean candidate (including multi-commit corrections). It preserves prior evidence for untouched areas and supplies only those exact revisions and bounded paths to the correction reviewer. Runtime counts `blocker|major` only when the report names an enforced changed path and links the finding to an accepted requirement index or a typed `security|data_loss` impact; out-of-scope discoveries are structurally downgraded to non-blocking backlog. The fourth valid correction cycle blocks, and trusted UI authorization permits exactly one additional cycle rather than reopening an unbounded review window. Missing or malformed reports remain `unknown`, never implicit clearance. Completion is vetoed until:
