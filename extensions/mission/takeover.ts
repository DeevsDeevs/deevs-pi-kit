...
// Cheap hint/guard listing for hot paths (mission_get, mission_create): reads only the small validated snapshots, never
// opens or replays session files. Marks usageComplete=false so any takeover attempted from a hint still fails closed on
// bounded budgets until the full scan derives exact usage.
export function listSnapshotTakeoverCandidates(ctx: ExtensionContext): MissionTakeoverCandidate[] {
	...
	return listMissionSnapshots(ctx.cwd)
		.filter((snapshot) => snapshot.owner.sessionId !== currentSessionId
			&& !["complete", "ended", "cleared"].includes(snapshot.mission.status))
...
export async function discoverMissionTakeoverCandidates(ctx: ExtensionContext): Promise<MissionTakeoverCandidate[]> {
	...
	const snapshots = listMissionSnapshots(ctx.cwd).filter((snapshot) => snapshot.owner.sessionId !== currentSessionId
		&& !["complete", "ended", "cleared"].includes(snapshot.mission.status));
	...
	for (const snapshot of snapshots) {
		...
		try {
			if (sourceSession?.id === snapshot.owner.sessionId) {
				...
				// Do not force usageComplete: a full replay of a readable source is complete only if the snapshot it
				// restored was itself complete. Forcing true would let carried incompleteness (from an earlier takeover
				// of an unreadable source) silently pass the bounded-budget gate.
...
export function selectMissionTakeoverCandidate(candidates: MissionTakeoverCandidate[], selector: string): MissionTakeoverCandidate {
	...
	if (!target) {
		...
		if (!only || candidates.length !== 1) {
			const ids = candidates.map((candidate) => candidate.snapshot.mission.missionId).join(", ");
			throw new Error(candidates.length
				? `Specify one Mission id: ${ids}`
				: "No Mission is available for takeover in this workspace.");
		}
		return only;
	}
	const matches = candidates.filter((candidate) => candidate.snapshot.mission.missionId === target
		|| candidate.snapshot.mission.slug === target);
	const [match] = matches;
	if (!match || matches.length !== 1) {
		throw new Error(matches.length
			? `Mission ${target} has ambiguous legacy session branches; resume the intended source session first.`
			: `Mission not found for takeover: ${target}`);
	}
	return match;
}