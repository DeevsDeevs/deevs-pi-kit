...
5. Commit a clean candidate before review. The first review persists exact repository heads. After changes are requested, Runtime anchors the complete clean correction (including multiple commits) to those accepted heads and persists exact changed paths; blocker/major then counts only when linked to an enforced path and accepted requirement or typed security/data-loss impact. Unrelated discoveries become follow-up work.
