...
- Never scrape panes, inject keystrokes, mutate focus, or use detached shell processes for coordination. Runtime launches every collaborator with `herdr agent start`; **the daemon's only native mail input is one idle-gated `herdr agent prompt` wake per target per 30 s, at most three per message, which names the sender and proves nothing**. Do not build launcher/proxy scripts to inject anything else.
...
- Every native collaborator, read-only included, receives the shared MCP connection through native CLI configuration and post-registration descriptor issuance; read-only Claude keeps only its file tools plus mail, read-only Codex stays in its read-only sandbox. Never infer MCP readiness from a held identity alone.
